 ...
 * 
 */
public abstract class ActionEditDescriptionView implements ResultView<String> {
	...
	public ActionEditDescriptionView(Activity activity, ActionService actionService) {
		this.activity = activity;
		this.actionService = actionService;
		this.view = activity.getLayoutInflater()
				.inflate(R.layout.action_detail_view_description_edit_text_layout, null);
	}
	
	public View build(final Action action) {
		final EditText editText = (EditText) view.findViewById(R.id.actionDetailViewDescriptionEditText);
		editText.setText(action.getDescription());
		
		final View button = view.findViewById(R.id.actionDetailViewDescriptionValidateButton);
		button.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				if (!Strings.isNullOrEmpty(editText.getText().toString())) {
					try {
						actionService.updateDescription(action.getId(), editText.getText().toString());
		...
		View undo = view.findViewById(R.id.actionDetailViewDescriptionUndoButton);
